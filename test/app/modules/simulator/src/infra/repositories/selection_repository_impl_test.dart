...
import 'package:futebol/app/modules/simulator/src/infra/repositories/selection_repository_impl.dart';
...
void main() {
  ...
  group("Selection Repository is working rightly", () {
    ...
    group("Method updateSelectionsStatistics is working", () {
      ...
      late List<Selecao> allSelections;

      setUpAll(() {
        selections = List.generate(2, (_) => FakeFactory.generateSelecao());
        allSelections = List.generate(4, (_) => FakeFactory.generateSelecao());
      });

      test("Returns an int when  no errors occur", () async {
        when(() => datasource.updateSelectionsStatistics(any(), any()))
            .thenAnswer((_) async => allSelections);

        var result =
            await repository.updateSelectionsStatistics(selections, 'A');
        expect(result.isRight(), isTrue);
        expect(result.fold(id, id), isA<List<Selecao>>());
        expect(result.fold(id, id), equals(allSelections));
      });
